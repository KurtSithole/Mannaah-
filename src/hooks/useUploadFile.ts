 ...
 *
 * @param signerOverride Sign the Blossom authorization with this signer instead
 * of the logged-in account's. Used by the in-app issue reporter on `/issues`,
 * where a report submitted "without your account" signs both the issue and its
 * screenshot upload with a throwaway key — authenticating the upload as the
 * real account would hand the Blossom server the very link the user asked not
 * to publish. When omitted, the logged-in account signs and a login is
 * required, as before.
 */
export function useUploadFile(signerOverride?: NostrSigner) {
  const { user } = useCurrentUser();
  const { config } = useAppContext();

  return useMutation({
    mutationFn: async (file: File) => {
      const signer = signerOverride ?? user?.signer;
      if (!signer) {
