...
interface ComposeBoxProps {
   ...
   * not poll mode, no custom publish). Replies, quotes, polls, comments, and
   * custom-kind publishes do not receive these tags regardless of this prop.
...
export function ComposeBox({
  ...
  const [editorShellMinHeight, setEditorShellMinHeight] = useState<number | undefined>();
  ...
  const resetComposeState = useCallback(() => {
    ...
    setEditorShellMinHeight(undefined);
  ...
  // is sized to fit its content immediately. While editing we only record
  // the shell's natural height for the next preview flip — minHeight is
  // applied solely in preview mode so the field can still shrink as text
  // is deleted.
  useEffect(() => {
    ...
    if (shell) {
      setEditorShellMinHeight(shell.offsetHeight);
    }
  }, [content, previewMode, expanded, forceExpanded, compact]);
  ...
  const mockEvent = useMemo(() => {
    ...
    // Preview the note as it will actually be published: Agora app links are
    // rewritten to nostr: URIs so the preview shows the same rich embeds
    // (campaign cards, mentions, quotes) the published note will render.
    const previewContent = rewriteAgoraLinksToNostrUris(content);

    const hashtags = extractHashtags(previewContent);
    ...
    if (customEmojis.length > 0) {
      ...
      let match;
      while ((match = shortcodeRegex.exec(previewContent)) !== null) {
    ...
    const mediaUrlMatches = previewContent.matchAll(new RegExp(IMETA_MEDIA_URL_REGEX.source, 'gi'));
  ...
  }, [user, voiceRecorder, uploadFile, buildContentWarningTags, customPublish, createEvent, onPublished, replyTo, queryClient, toast, onSuccess, statsPubkey, t]);

  const handleSubmit = async () => {
    if (!content.trim() || !user || charCount > MAX_CHARS) return;

    try {
      // Rewrite Agora app links (agora.spot/:nip19) into canonical nostr:
      // URIs so the published note carries portable references that any
      // Nostr client can resolve — and so mention/quote tag extraction below
      // sees the npub/note/naddr identifiers.
      const rewrittenContent = rewriteAgoraLinksToNostrUris(content);
      const hashtags = extractHashtags(rewrittenContent);
      const tags: string[][] = hashtags.map((t) => ['t', t]);

      // NIP-27 mention p tags — extract nostr:npub1... from content
      const mentionMatches = rewrittenContent.matchAll(/nostr:(npub1[023456789acdefghjklmnpqrstuvwxyz]+)/g);
      ...
      let finalContent = rewrittenContent.trim();
    ...
    <div className={cn(
      ...
      <div className={cn("flex gap-3", forceExpanded && "flex-1 min-h-0")}>
        ...
        <div className={cn("flex-1 min-w-0", forceExpanded && "flex flex-col min-h-0")}>
          ...
          {/* Shared shell keeps a stable min-height only while previewing so
              pure-text posts don't jump when NoteContent metrics differ from
              the textarea's scrollHeight. In edit mode the shell follows the
              textarea so removable text shrinks the field again. Preview may
              still grow taller when embeds render larger than the typed text. */}
          <div
            ref={editorShellRef}
            className="relative"
            style={
              previewMode && editorShellMinHeight
                ? { minHeight: editorShellMinHeight }
                : undefined
            }
          >
          {!previewMode ? (
          /* ── Edit mode — Textarea ────────────────────────────── */
            <textarea
              ...
              className={cn(
                'w-full bg-transparent text-foreground placeholder:text-muted-foreground resize-none outline-none text-lg leading-normal pt-2.5 pb-2 opacity-85 break-words overflow-hidden transition-[min-height] duration-200 ease-in-out',
          ...
          mockEvent && (
            <div className="pt-2.5 pb-2 overflow-hidden">
              <div className="text-lg leading-normal opacity-85 [&_img]:max-w-full [&_img]:h-auto">
        ...
        {/* Char budget — sits where the old "Post to" destination row was
            (above the toolbar, right-aligned). Only shows once ≤100
            characters remain so it doesn't clutter the compose area. */}
        {isExpanded && mode === 'post' && remaining <= 100 && (
          <div className="mt-2 flex items-center justify-end gap-1.5">
            <CharRing count={charCount} max={MAX_CHARS} />
            <span
              className={cn(
                'text-xs tabular-nums',
                remaining < 0 ? 'text-destructive font-semibold' : 'text-amber-500',
              )}
            >
              {remaining}
            </span>
            ...
            <div className={cn("flex items-center justify-between mt-3", forceExpanded && "shrink-0")}>
              ...
              <div className="flex items-center gap-1">
                ...
                <Popover open={trayOpen} onOpenChange={setTrayOpen}>
                  ...
                  <PopoverContent side="top" align="start" sideOffset={6} className="w-48 p-1.5 rounded-xl border-border shadow-lg">
                    <div className="flex flex-col gap-0.5">
                      {voiceRecorder.isSupported && (
                        <button
                          type="button"
                          onClick={() => { handleStartRecording(); setTrayOpen(false); }}
                          disabled={!user}
                          className="flex items-center gap-2.5 w-full px-3 py-2 rounded-lg text-sm transition-colors text-muted-foreground hover:text-foreground hover:bg-secondary/60 disabled:opacity-40"
                        >
                          <Mic className="size-4" /><span className="font-medium">{t('compose.voice.voiceMessage')}</span>
                        </button>
                      )}
              ...
              {/* Right: preview + post/poll button */}
              <div className="flex items-center gap-3">
                {content.trim().length > 0 && controlledPreviewMode === undefined && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={() => setInternalPreviewMode((v) => !v)}
