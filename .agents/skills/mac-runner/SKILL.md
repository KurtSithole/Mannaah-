...
| `vite.config.ts: Unexpected token 'c', "concurrent"... is not valid JSON` | GitLab Runner sets `CONFIG_FILE=/Users/alex/.gitlab-runner/config.toml` in the job environment (its TOML starts with `concurrent = 1`), which collides with vite's build-config lookup in `loadBuildConfig` | Already fixed: use `AGORA_CONFIG_FILE` for the override env var. Regressed once when the Ditto→Agora rebrand reverted the name to `CONFIG_FILE` — keep it namespaced. |
